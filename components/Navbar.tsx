...
export default function Navbar (){
    ...
    return (
        <main className="flex items-center justify-between shadow-sm md:px-10 md:py-2 max-md:p-2 z-50 relative">
            ...
            <div className={`lg:hidden bg-white h-screen absolute top-0 right-0 flex flex-col gap-6 pt-20 w-full ${navOpen ? "flex" : "hidden"}`}>
                ...
                <Link href={"#"} className="flex items-center text-2xl gap-1 bg-[#2E2910] text-white px-3 rounded-sm border border-[#2E2910] py-1 group"><FiUser className="group-hover:-translate-x-0.5 transition-all duration-200" /> Register</Link>
            </div>
            </div>
        </main>
    )
