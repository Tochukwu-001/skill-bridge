  ...
  return (
    <main className="flex items-center justify-between shadow-sm md:px-10 md:py-4 max-md:p-2 z-50 relative">
      ...
      <div className="flex items-center gap-5 max-lg:hidden">
        <Link
          href={"/advertise"}
          className="text-xl border px-3 rounded-sm py-1 hover:bg-[#EBE3A7] transition-all duration-200"
        >
          Advertise Skills
        </Link>
        {session ? (
          ...
          <Link
            href={"/auth"}
            className="flex items-center text-xl gap-1 bg-[#2E2910] text-white px-3 rounded-sm border border-[#2E2910] py-1 group"
          >
      ...
      <div
        className={`lg:hidden bg-white h-screen absolute top-0 right-0 flex flex-col gap-6 pt-20 w-full ${navOpen ? "flex" : "hidden"}`}
      >
